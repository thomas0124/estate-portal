import type { StatusColorConfig } from "./types"

export const DEFAULT_STATUS_COLORS: StatusColorConfig = {
  仲介物件: "bg-blue-500/10 text-blue-700 dark:text-blue-400 border-blue-500/20",
  業者物件: "bg-green-500/10 text-green-700 dark:text-green-400 border-green-500/20",
  所有物件: "bg-orange-500/10 text-orange-700 dark:text-orange-400 border-orange-500/20",
  契約後: "bg-success/10 text-success border-success/20",
  販売中止: "bg-muted text-muted-foreground border-border",
}

export const DEFAULT_OWNED_PROPERTY_CARD_COLOR = "#fed7aa" // Default hardcoded color for owned property card
